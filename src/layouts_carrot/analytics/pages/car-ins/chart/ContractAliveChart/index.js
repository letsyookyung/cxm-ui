...
const ContractAliveChart = ({
  ...
  const [title, setTitle] = useState("계약유지여부");
  ...
  useEffect(() => {
    if (isSuccess) {
      ...
      setTitle(`계약유지여부 (전체: ${countTotal.toLocaleString()})`);
      ...
      if (isFirst) {
        setUndCrYnArray(nameList.map((item) => {
...
export default ContractAliveChart;