...
const CrPrdCmpcdNmChart = ({
  params,
  setCrPrdCmpcdNmArray,
}) => {
  const path = `${apiURL}/car-manufacture`
  ...
  const [title, setTitle] = useState("제조사");
  ...
  useEffect(() => {
    if (isSuccess) {
      ...
      setTitle(`제조사 (전체: ${countTotal.toLocaleString()})`);
      ...
      if (isFirst) {
        setCrPrdCmpcdNmArray(nameList.map((item) => {
  ...
  return (
    <VerticalBarChart
      icon={{ color: "dark", component: "factory" }}
      title={title}
      height="25rem"
      description=""
...
export default CrPrdCmpcdNmChart;